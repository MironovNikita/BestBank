...
- 🔍 elasticsearch - это распределённая поисковая и аналитическая система. Предназначена для хранения, поиска и анализа больших объёмов данных в реальном времени;
- 📊 grafana - открытая платформа для сбора, мониторинга и визуализации данных, ориентированная на данные систем ИТ-мониторинга.
- 📋 kibana - веб-платформа для визуализации и управления данными, хранящимися в Elasticsearch (системе поиска и анализа данных).
- 🚛 logstash - инструмент для сбора, обработки и трансформации данных из журналов в разных системах. Является частью экосистемы Elastic Stack (ELK).
- ⚡ prometheus - система мониторинга и оповещения с открытым исходным кодом, которая в реальном времени собирает и анализирует метрики работы приложений и серверов;
- 🕸️ zipkin - система трейсинга и мониторинга запросов, позволяющая собирать данные о времени выполнения сервисов в распределённых системах
...
## Системы мониторинга
### Zipkin
В каждом микросервисе включён трейсинг: сервис создаёт traceId и spanId для каждого запроса. Данные метки добавляются в заголовки запросов. Благодаря им zipkin хранит эти спаны и строит цепочку запроса через все сервисы.
Что видно в Zipkin:
- история запроса по всем сервисам;
- длительность каждого вызова (span);
- в каком месте были ошибки или задержки исполнения запросов.

Пример логов с traceId и spanId:
```java
2026-01-21 - 18:35:55.778 (Z)  INFO 1 ---> [69711c8b66a795ec92a1252c7976f701/92a1252c7976f701] [kafka-producer-network-thread | producer-1] c.b.service.NotificationsServiceImpl : Уведомление успешно отправлено на email: ***, topic: transfers-notifications, offset: 0
2026-01-21 - 18:35:55.779 (Z)  INFO 1 ---> [69711c8b66a795ec92a1252c7976f701/92a1252c7976f701] [kafka-producer-network-thread | producer-1] c.bank.service.TransfersServiceImpl : Перевод с ID 1 на ID 2 успешно сохранён.
```

Так выглядит Dependencies-раздел при использовании функционала приложения
<p align="center">

  <img src="https://github.com/MironovNikita/BestBank/blob/main/images/zipkin-management.png">

</p>

### Prometheus
Все микросервисы экспортируют метрики через /actuator/prometheus (Spring Boot + Micrometer). Prometheus скрейпит (запрашивает) эти метрики раз в определённое время. Prometheus хранит данные в TSDB (time series database).

Какие метрики отслеживаются в Prometheus:
- QPS, latency, ошибки по каждому сервису;
- использование ресурсов JVM, GC, CPU, памяти;
- метрики Kafka, PostgreSQL и т.д.

Также отслеживаются кастомные метрики:
- количество успешных/неуспешных логинов пользователей;
- количество успешных/неуспешных (заблокированных) попыток перевода денег как между своими счетами, так и между счетами различных пользователей;
- количество успешных/неуспешных (заблокированных) попыток операций с наличными;
- количество успешных/неуспешных уведомлений пользователей через email;
- количество успешных/неуспешных обновлений курсов валют.

### Grafana
Grafana подключается к Prometheus как data source. Дашборды были настроены по указанным выше метрикам: дэшборд безопасности и дэшборд бизнес-метрик приложения.

Дэшборд безопасности приложения: 
<p align="center">

  <img src="https://github.com/MironovNikita/BestBank/blob/main/images/metrics-security.png">

</p>

Дэшборд финансовых метрик: 
<p align="center">

  <img src="https://github.com/MironovNikita/BestBank/blob/main/images/metrics-financial.png">

</p>

### Logstash
Сервисы пишут логи через Logback → Kafka → Logstash. Сам logstash использует конфигурацию logstash.conf:
- input: откуда читать логи (Kafka);
- filter: осуществляет преобразование логов - добавление тегов: уровня логов, индексация по времени;
- output: куда отправлять логи для сохранения - Elasticsearch.

Logstash может обогащать логи метаданными (traceId, spanId, service name).

### Elasticsearch
Logstash или другие агенты (например, Kafka) отправляют логи в Elasticsearch. Elasticsearch хранит их в индексах (например, bestbank-logs-2026.01.21 - настраивается в конфиге). Метаданные логов (уровень, сервис, traceId, spanId) индексируются для быстрого поиска. По итогу Elasticsearch позволяет собрать все логи сервисов в одном месте, сортируя их по по времени, уровню (INFO/DEBUG/ERROR), сервису, traceId и т.д.

### Kibana
Kibana подключается к Elasticsearch, создаёт индексы и дашборды для логов (bestbank-logs-*). Позволяет:
- фильтровать логи по сервису, уровню, traceId и т.д;
- смотреть логи в реальном времени;
- создавать визуализации, графики частоты ошибок, распределения по уровню INFO/DEBUG/ERROR.

После запуска helm-чарта в kibana необходимо:
1) Создать Data View
2) Далее переходим по Stack Management → Data Views → Create data view
Заполняем:
- Name: bestbank-logs
- Index pattern: bestbank-logs-*
- Time field: @timestamp
3) Сохраняем
4) Чтобы посмотреть логи, переходим в Analytics → Discover
Теперь можно фильтровать логи по метрикам, указанным ниже:

<p align="center">

  <img src="https://github.com/MironovNikita/BestBank/blob/main/images/kibana-services.png">

</p>
----
<p align="center">

  <img src="https://github.com/MironovNikita/BestBank/blob/main/images/kibana-levels.png">

</p>

Сами логи выглядят следующим образом:
<p align="center">

  <img src="https://github.com/MironovNikita/BestBank/blob/main/images/kibana.png">
